'''
    原模型：
        ...
        直接拟合软标签，没有对真值y进行拟合
    蒸馏后:
        [Test1] Accuracy 0.94570
        [Test2] Accuracy 0.83630
        [Test3] Accuracy 0.93310
        [Test4] Accuracy 0.93810
        [Test5] Accuracy 0.93860
        [Test6] Accuracy 0.85560
        [Test7] Accuracy 0.85490
'''
...
with tf.name_scope("build_network"):
    ...
    loss = 20*20*tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_prob, logits=y_))
    predict = tf.argmax(y_, 1)
    correct_prediction = tf.equal(predict, tf.argmax(y_p, 1))
...
with tf.name_scope("train"):
    ...
    with tf.Session() as sess:
        ...
        prediction, _ = sess.run([predict, train_op],
                                          feed_dict={
                                              x: X_test,
                                              y_p: np.zeros((10000, 10))
                                          })
        acc_test = np.mean(np.equal(prediction, y_test))
